...
pair=[[5,6],[5,7],[5,11],[7,9],[6,8],[6,12],[8,10],[11,12],[11,13],[13,15],[12,14],[14,16]]
AL_FRAME=20
COUNTER=0
...
while True:
    ...
    for i, position in enumerate(key_point_positions):
            ...
            if(confidenceScores[i]>0.3 and i<5):
                if(confidenceScores[i]>head_c):
                    head_c=confidenceScores[i]
                    head_p=int(y_coords[i])
            elif(confidenceScores[i]>0.3 and i>12):
                if(confidenceScores[i]>foot_c):
                    foot_c=confidenceScores[i]
                    foot_p=int(y_coords[i])
            if(confidenceScores[i]>0.3):
                cv2.circle(frame, (x,y), 5, (0, 255 ,0), cv2.FILLED)
    for i in range(12):
        index0=[np.array(pair[i][0])]
        index1=[np.array(pair[i][1])]
        A=np.int32(index0)
        B=np.int32(index1)

        if(confidenceScores[int(A)] and confidenceScores[int(B)]>0.3):
            x1=int(x_coords[int(A)])
            y1=int(y_coords[int(A)])
            x2=int(x_coords[int(B)])
            y2=int(y_coords[int(B)])
            cv2.line(frame, (x1,y1),(x2,y2), (255, 255, 0),3, cv2.FILLED)
    if(abs(foot_p-head_p)<=80 and head_c!=0 and foot_c!=0):
            cv2.putText(frame,'FALL',(30,200),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2,cv2.LINE_AA)
    elif(foot_p==0 or head_p==0):
            cv2.putText(frame,'NO DETECT',(30,200),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
    else:
